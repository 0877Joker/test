...
<tr align='center' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='#FCFDEE';" onMouseOut="javascript:this.bgColor='#FFFFFF';" height="22" >
	...
	<td>
  	...
  	<c:if test="${bean.shenhe!='确认报名'}">
  	<c:if test="${bean.shenhe!='取消报名'}">
  	<a href="${url2 }delete?id=${bean.id }">取消报名</a>
  	</c:if>
  	</c:if>
  	<c:if test="${bean.shenhe=='照片审核不通过'}">
  
  	<a href="method!baokaoupdate11?id=${bean.id }">重新审核</a>
